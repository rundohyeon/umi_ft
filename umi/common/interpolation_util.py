...
def get_interp1d(t, x):
    t = np.asarray(t)
    x = np.asarray(x)
    if t.size == 0 or np.asarray(x).size == 0:
        raise ValueError("get_interp1d: empty t or x (need at least one sample).")
    if len(t) < 2:
        raise ValueError("get_interp1d: need at least 2 time samples for interpolation.")
