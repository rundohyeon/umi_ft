...
class UvcCamera(mp.Process):
    ...
    @property
    def is_ready(self):
        # Match robot controller: if the capture process dies (e.g. uncaught
        # queue.Full before recovery), ready_event may still be set.
    ...
    def run(self):
        ...
        if not cap.isOpened():
            raise RuntimeError(
                f"Failed to open camera {self.dev_video_path}. "
                "If Elgato/HDMI in Docker: bash docker/container.sh attach-video"
            )
        ...
        try:
            ...
            # Elgato capture cards (HD60/Cam Link) deliver uncompressed YUYV by
            # default; at 1080p60/4K30 that exceeds practical USB throughput and
            # cap.grab() succeeds while retrieve() silently never returns a real
            # frame (process spins forever, never reaching ready). MJPG must be
            # requested before resolution/fps for V4L2 to negotiate the right mode.
            ...
            print(
                f"[UvcCamera {self.dev_video_path}] opened "
                f"requested={w}x{h}@{fps} actual="
                f"{cap.get(cv2.CAP_PROP_FRAME_WIDTH):.0f}x"
                f"{cap.get(cv2.CAP_PROP_FRAME_HEIGHT):.0f}@"
                f"{cap.get(cv2.CAP_PROP_FPS):.1f} "
                f"fourcc={int(cap.get(cv2.CAP_PROP_FOURCC)):#x}",
                flush=True,
            )
            ...
            while time.time() < warmup_deadline:
                ...
                raise RuntimeError(
                    f"Camera {self.dev_video_path} opened but grab() returned no frames. "
                    "Check HDMI/GoPro signal, USB cable, and Docker video devices "
                    "(bash docker/container.sh attach-video)."
                )
            ...
            while not self.stop_event.is_set():
                ...
                if iter_idx == 0:
                    print(
                        f"[UvcCamera {self.dev_video_path}] first retrieve "
                        f"shape={frame.shape}",
                        flush=True,
                    )
                    expected_h, expected_w = self.resolution[::-1]
                    actual_h, actual_w = frame.shape[:2]
                    if (actual_h, actual_w) != (expected_h, expected_w):
                        print(
                            f"[UvcCamera {self.dev_video_path}] warning: "
                            f"requested {expected_w}x{expected_h}, "
                ...
                if self.transform is not None:
                    if iter_idx == 0:
                        print(
                            f"[UvcCamera {self.dev_video_path}] applying transform",
                            flush=True,
                        )
                    put_data = self.transform(dict(data))
                    if iter_idx == 0:
                        print(
                            f"[UvcCamera {self.dev_video_path}] transform done "
                            f"color_shape={put_data['color'].shape}",
                            flush=True,
                        )
                ...
                if iter_idx == 0:
                    print(
                        f"[UvcCamera {self.dev_video_path}] ready",
                        flush=True,
                    )
