...
@click.option(
    ...
    show_default=True,
    help=(
        "If robot_config has no gripper entry, control the Dynamixel directly "
        "from --dynamixel_gripper_config while keeping UmiEnv in no-gripper mode."
    ),
)
@click.option(
...
def main(input, output, robot_config,
    ...
    local_dataset_path = _PROJECT_ROOT.joinpath(
        "artifacts", "0709_robot_tcp", "dataset_robot_tcp.zarr.zip")
    if not local_dataset_path.is_file():
        raise click.ClickException(
            f"Self-contained training Zarr is missing: {local_dataset_path}"
        )
    cfg.task.dataset.dataset_path = str(local_dataset_path)
    print("checkpoint dataset_path metadata:", embedded_dataset_path)
    print("runtime dataset_path (local):", cfg.task.dataset.dataset_path)
    ...
    with SharedMemoryManager() as shm_manager:
        ...
        with sm_ctx as sm, direct_gripper_cm as direct_gripper, UmiEnv(
            ...
            has_gripper_control = (not no_gripper) or (direct_gripper is not None)
            ...
            with torch.no_grad():
                policy.reset()
                obs_for_model = _apply_slam_frame_fix_to_obs(obs, n_robots)
                    ...
                    while True:
                        ...
                        if key == 27:  # Esc
                            ...
                            _print_match_pose_compare(
                                ...
                                pose,
                                live_tcp6,
                                will_move=match_g_move_robot,
                            )
                            if match_g_move_robot:
                                duration = 3.0
                                grip = float(
